...
public class LevelManagerScript03 : MonoBehaviour 
{
	public static bool isLevel03; 
	private static LevelManagerScript03 mInstance;

	public static LevelManagerScript03 Instance
	{
		get
		{
			if(mInstance == null)
			{
				...
				if(tempObject == null)
				{
					...
					mInstance = obj.AddComponent<LevelManagerScript03>();
					obj.tag = "LevelManager";
				}
				else
				{
					mInstance = tempObject.GetComponent<LevelManagerScript03>();
				}
			}
			return mInstance; 
		}
	...
	void Update () 
	{
		...
		{
			GameObject.Find("Level3").GetComponent<Text>().enabled = false;
		}
		...
		if(spawnEnemy.enemyCounter <= 0)
		{
			...
			{
				GameObject.Find ("ChangeLevel").GetComponent<ChangeLevelScript>().loadLevel04 = true;
